...
//     std::cout << "templateRef constructor: theHist=" << theHist << " theColor=" << theColor << std::endl;
  ...
  void normalize(const int centralMode = 1) {
    if (hist->GetSumOfWeights() >0) {
      ...
      float scaleFacCentral = 1. / hist->GetSumOfWeights();
	...
	if (centralMode == 0) {
	  if (tempPlus[iNuisance]->GetSumOfWeights() >0) {
	    scaleFac = 1. / tempPlus[iNuisance]->GetSumOfWeights();
	  } else {
	...
	if (centralMode == 0) {
	  if (tempMinus[iNuisance]->GetSumOfWeights() >0) {
	    scaleFac = 1. / tempMinus[iNuisance]->GetSumOfWeights();
	  } else {
  ...
  void addNuisance(TH2F* theTempPlus, TH2F* theTempMinus, float theNSigma) {
    ...
    if ( (theNSigma !=0) && (hist->GetSumOfWeights() != 0) ) {
      float theNormSlope = (theTempPlus->GetSumOfWeights() - theTempMinus->GetSumOfWeights()) / 
	( 2 *  theNSigma * hist->GetSumOfWeights() );
...
// 	      << "   upper " << theTempPlus->GetName() << "  " << theTempPlusCopy->GetSumOfWeights() << std::endl
// 	      << "   lower " << theTempMinus->GetName() << "  " << theTempMinusCopy->GetSumOfWeights() << std::endl;
    ++nNuisance;
  }
  // copy constructor
...
//     nSigma = otr.nSigma;
  ...
  void Print() {
    std::cout << "---- Template " << name << "  base hist " << hist->GetName()
	      << " norm " << hist->GetSumOfWeights() << std::endl;
    for (int iNuisance=0; iNuisance<nNuisance; ++iNuisance) {
      std::cout << "      " << "Nuisance parameter " << iNuisance << std::endl;
      float varPlus = -1;
      float varMinus = -1;
      if (hist->GetSumOfWeights() != 0) {
	varPlus = tempPlus[iNuisance]->GetSumOfWeights() / hist->GetSumOfWeights();
	varMinus = tempMinus[iNuisance]->GetSumOfWeights() / hist->GetSumOfWeights();
      }
      std::cout << "          " << "Upper: " << std::setw(40) << tempPlus[iNuisance]->GetName() 
		<< "  " << tempPlus[iNuisance]->GetSumOfWeights() 
		<< " ratio " << varPlus << std::endl;
      std::cout << "          " << "Lower: " << std::setw(40) << tempMinus[iNuisance]->GetName() 
		<< "  " << tempMinus[iNuisance]->GetSumOfWeights() 
...
double getSystBbpurTpat( const string theScenario, const int theSignalMass ) {
  const int mSignal = 10;
  int masses[mSignal] = {90, 100, 120, 130, 140, 160, 180, 200, 250, 350};
...
// 	templates.push_back(  templateRef("H",mjjEbtsignal[ibtag],0.001,2) );
	...
	gTemplateFitter->Constrain(iTemplate+1,0.0,2*gTemplateFitter->data->GetSumOfWeights() / 
				   gTemplateFitter->getTemplateHist(iTemplate)->GetSumOfWeights());
      }
    }
    ...
    // 
    theSummedTemplateHist->Scale( mjjEbtdata[ibtag]->GetSumOfWeights());
