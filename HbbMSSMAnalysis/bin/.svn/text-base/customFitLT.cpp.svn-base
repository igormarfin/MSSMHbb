...
float getEfficiencyFactor(const float& x0,const float& sigma, TRandom3& rdm) {
  float fNew = rdm.Gaus( x0, sigma );
  if (fNew<0) return 0;
  return fNew;
}
...
//   std::string systName[nSyst] = { "JES", "SFbc", "SFudsg" };
  ...
  TH2F* hTotalTemplateCentral[nbtag];
  TH2F* hTotalTemplateSyst[nSyst][nUpDown][nbtag];
  ...
  TFile* fTotalBG[nbtag];

  for (int ibtag=0; ibtag<nbtag; ++ibtag) {

    // open the total template file
    string totalBGName(Form("/afs/naf.desy.de/user/r/rmankel/scratch/HbbPat/CMSSW_4_2_4_patch1/src/Analysis/HbbMSSMAnalysis/test/TotalBackgroundNuisance/%s/TotalT_CSVT_BGonly.root",scenario.c_str()));

    std::cout << "totalBG name: " << totalBGName << std::endl;
    fTotalBG[ibtag] = new TFile( totalBGName.c_str() );
    if (fTotalBG[ibtag] == NULL) {
      std::cout << "Could not open " << totalBGName << std::endl;
      return;
    }
...
//     markerFile.close();
    ...
    // read the total template
    std::string totalTemplateCentralName( Form("TotalBG_BGonly_%s_SimErrors",sbtag[ibtag].c_str()) );
    hTotalTemplateCentral[ibtag] = (TH2F*) fTotalBG[ibtag]->Get(totalTemplateCentralName.c_str());
    if (hTotalTemplateCentral[ibtag]  == NULL) {
      std::cout << "Histogram not found: " << totalTemplateCentralName.c_str() << std::endl;
      return;
    }
    hTotalTemplateCentral[ibtag]->SetLineColor(6);
    hTotalTemplateCentral[ibtag]->SetLineWidth(3);
    
    // read the total template syst templates
    for (int iSyst=0; iSyst<nSyst; ++iSyst) {
      for (int iUpDown=0; iUpDown<nUpDown; ++iUpDown) {
	std::string totalTemplateSystname( Form("TotalBG_BGonly_%s_%s_%s",sbtag[ibtag].c_str(),
						systName[iSyst].c_str(),
						upDownName[iUpDown].c_str()) );
	hTotalTemplateSyst[iSyst][iUpDown][ibtag] = (TH2F*) fTotalBG[ibtag]->Get(  totalTemplateSystname.c_str() );
	if (hTotalTemplateSyst[iSyst][iUpDown][ibtag] == NULL) {
	  std::cout << "Histogram not found: " << totalTemplateSystname << std::endl;
	  return;
	}
      }
    }
...
// 		  << "  " << tName.name() << std::endl;
  ...
  for (int ibtag = 0; ibtag < nbtag; ++ibtag) {
    ...
    // draw Ebtdata and total template in comparison
    mjjEbtdataProX[ibtag]->Draw();
    TH1D* totalPro = hTotalTemplateCentral[ibtag]->ProjectionX(Form("%sProX",hTotalTemplateCentral[ibtag]->GetName()),0,-1,"e");
    totalPro->SetLineColor(6);
//     totalPro->Draw("SAME");
//     canvas->Print("DataTotalComp.png");
    ...
    TH1D* HFractionE_H = new TH1D("HFractionE_H","Higgs Fraction error",nBins,0,+5*sFrac);
    ...
    TH1D* HXSectE_H = new TH1D("HXSectE_H","Higgs XSect Error",nBins,0,+5*sXSect);
...
// 		<< mjjEbtTemplateMerge01[1][ibtag]->hist->GetRMS() << std::endl;
	...
	if ( activeNuisance[iNuisance] ) ++nActiveNuisance;
      ...
      bool useTotalTemplate = true;

      if (useTotalTemplate) {
	mjjEbtdataRdm[ibtag] = new TH2F( *hTotalTemplateCentral[ibtag] );
      } else {
	...
	for (int biny=1; biny<=mjjEbtdataRdm[ibtag]->GetYaxis()->GetNbins(); ++biny) {
	  float oldContent = mjjEbtdataRdm[ibtag]->GetBinContent(binx,biny);
	  float newContent = rdm.Poisson( mjjEbtdataRdm[ibtag]->GetBinContent(binx,biny) );
	  mjjEbtdataRdm[ibtag]->SetBinContent(binx,biny,newContent);
	  float oldError = mjjEbtdataRdm[ibtag]->GetBinError(binx,biny);
	  float newError = 0;
	  if (mjjEbtdataRdm[ibtag]->GetBinContent(binx,biny)>=0) {
	    newError = sqrt( mjjEbtdataRdm[ibtag]->GetBinContent(binx,biny) );
	  }
	  mjjEbtdataRdm[ibtag]->SetBinError(binx,biny,newError);
	  std::cout << " binx=" << binx << " biny=" << biny
		    << " oldContent=" << oldContent
	...
	if ((*gTemplateFitter->templates)[iTemplate].name != "H" || constrainSignal) {
      ...
      mjjEbtdataRdm[ibtag]->SetMarkerStyle(20);
      mjjEbtdataRdm[ibtag]->GetXaxis()->SetTitle("m(Jet1 Jet2) [GeV/c^{2}]");
      mjjEbtdataRdm[ibtag]->GetYaxis()->SetTitle("EvtBTag");
      mjjEbtdataRdm[ibtag]->GetZaxis()->SetTitle("N / 10 GeV/c^{2}");
      mjjEbtdataRdm[ibtag]->ProjectionX()->Draw("Ep");
      double value = 0;
      double error = 0;
      double fPenalty = -1;

      int iH = -1;  // pointer to Higgs template
      for (int ii=0; ii<gTemplateFitter->nTemplates(); ++ii) {
	...
	fPenalty =  HFractionError[ibtag][iSignal] * mjjEbtdataRdm[ibtag]->GetSum() / sqrt( mjjEbtdataRdm[ibtag]->GetSum() );
	...
	HSeen[ibtag][iSignal] = HFraction[ibtag][iSignal] * mjjEbtdataRdm[ibtag]->GetSum();
	HSeenError[ibtag][iSignal] = HFractionError[ibtag][iSignal] * mjjEbtdataRdm[ibtag]->GetSum();
	...
	float fLuminosity = getEfficiencyFactor( 1., 0.022, rdm );
	float fTrigger = getEfficiencyFactor( 1., 0.10, rdm );
	float fOnlineBtag = getEfficiencyFactor( 1., 0.3225 , rdm);
	float fTopology = getEfficiencyFactor( 1., 0.06 , rdm);

	float theEfficiencyRdm = efficiency[iSignal][ibtag] 
	  * fLuminosity * fOnlineBtag * fTrigger * fTopology;
	...
	HCorr[ibtag][iSignal] = HSeen[ibtag][iSignal] / theEfficiencyRdm;
	HCorrError[ibtag][iSignal] = HSeenError[ibtag][iSignal] / theEfficiencyRdm;
