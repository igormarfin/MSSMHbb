...
L25BtagTriggerObjectFilterList.push_back("hltBLifetimeL25FilterbbPhi1stTrackL1FastJetFastPV");
  ...
  float xsectExternal =0;
  //int nInputEvents= getHbbMetadata(theFileColl,genericTriggerList,HLTBtagTriggerObjectList,1000.,lumiScaleFac,_doMC);
  int nInputEvents= getHbbMetadata(theFileColl,genericTriggerList,HLTBtagTriggerObjectList,1000.,lumiScaleFac,_doMC,xsectExternal);
...
///2012 
  ...
  ///2011 Medium
  #if defined(MEDIUM2011)

  bTagEffOffline = new bTagEff("/afs/naf.desy.de/user/r/rmankel/scratch/HbbPat/CMSSW_4_2_3_patch2/src/DesyHiggsAnalyses/HBBAnalyses/test/results/job014/QCD_Pt-N14/btagMatrixOffline-csv/plotmistag-b.root","offline",flavlabel,sbtag,3,nbtag);
  bTagReleffOnline  = new bTagEff("/afs/naf.desy.de/user/r/rmankel/public/btagMatrix/btagMatrixOnline-V4.root","online",flavlabel,sbtag,3,nbtag);

  #endif



//std::cout<<"After constructors of Btagging "<<std::endl;
...
hout->mkdir("mvaCount");
hout->cd("mvaCount");
TH1 * _mvacount= new TH1F("mvaCount","mvaCount",2,0,4);
hout->cd();

hout->mkdir("xsect");
hout->cd("xsect");
TH1 * _xsect= new TH1F("xect","xsect",1,0,2);
hout->cd();
...
//_pileup = new PileUpProducer("PUProducer","PUProducer","MyDataPileupHistogram.root","PileUp_Summer12.root","pileup");
_pileup = new PileUpProducer("PUProducer","PUProducer",PU_data.Data(),PU_mc.Data(),"pileup");
...
_xsect->SetBinContent(1,xsectExternal);
...
double maxEtaMVA = 3.2;
std::vector<int> _theFlav; /// to catch flavor of objects
std::vector<int> _leading; /// to store the index of leading jets
...
double _puwgt=1.0;
#if defined(PU)
	_puwgt=(_pileup)?_pileup->GetWeight(nPUTruth):1.0;
#endif
...
///_MVA_BEGIN_ getting of the first nSelJet-1 th objects' info 
       ...
       #if defined (MEDIUM2011)
	 _theFlav.push_back(jetFlavorCodeNewCat2011flavdef(iJet)); 
	#else
	 _theFlav.push_back(jetFlavorCodeNewCat(iJet)); 
	#endif
       _leading.push_back(iJet);
//      std::cout<<" jet # "<<myJet<<" with flav "<<jetFlavorCodeNewCat(iJet)<<" with pT "<<ptJet[iJet]<<std::endl;
	  _vecJets.push_back(math::XYZTLorentzVector(pxJet[iJet], pyJet[iJet], pzJet[iJet], energyJet[iJet]));
//      std::cout<<" XYZLorentzVector pT "<< _vecJets.back().Pt()<<std::endl;
	  _isplaced=true;
//_MVA_END_ 	
      }
...
//          std::cout<<" additional jet # "<<myJet<<" with flav "<<jetFlavorCodeNewCat(iJet)<<" with pT "<<ptJet[iJet]<<std::endl;
		 _vecJets.push_back(math::XYZTLorentzVector(pxJet[iJet], pyJet[iJet], pzJet[iJet], energyJet[iJet]));
//          std::cout<<" XYZLorentzVector pT "<< _vecJets.back().Pt()<<std::endl;
...
//std::cout<<"I pass first preselection"<<std::endl;
# if !defined (NO_DR23)
if ( !(deltaR12 > 1.0 && deltaR23 > 1.0 && deltaR13 > 1.0 && deltaEta < deltaEtaCut12) ) continue;
# endif

//std::cout<<"I pass second preselection"<<std::endl;
...
// cout<< "lumiScaleFac="<<lumiScaleFac<<endl;
...
//cout<<"PU weight is "<<_pileup->GetWeight(nPUTruth)<<endl;
...
cout<<"PU from Alexei weight is "<< pileupWeight->PUWeightS6(nPUTruth)<<endl;
}
...
//     trgSelect = trgAccept & trigSelector.mask( run );
//     trgAccept=trgSelect;
...
cout<<"_SF_2="<<_SF_2<<" for jet "<<iJ<<" with systype "<<systype<<" with  variation"<<_sfvariation<<" with flavor"<<_theFlav[iJ]<<" for ibtag "<<ibtag <<endl;


_SF_MC[ibtag]*=_SF_2;

cout<<"_SF_MC[ibtag]="<<_SF_MC[ibtag]<<endl;
...
//          cout<<"_offlineBtagWeight ="<<_offlineBtagWeight<<" iJ="<<iJ<<endl;
		}
...
//   cout<<"_onlineBtagWeight="<<_onlineBtagWeight<<endl;
...
//cout<<"_SF_MC[iiovl] = "<<_SF_MC[iiovl]<<endl;
...
if (_isOk) _afterMVA++;
  ...
  continue;
    ...
    if (forceTripleOnlineBtagMatch) {
      ...
      if (nJet>=nSelJet ) {
    tripleOnlineBtagMatchOK2 = (HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[0]],isJetWithL25JetBitPattern[leadingJets[0]],run) && 
  HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[1]],isJetWithL25JetBitPattern[leadingJets[1]],run))
      || (HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[0]],isJetWithL25JetBitPattern[leadingJets[0]],run) && HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[2]],isJetWithL25JetBitPattern[leadingJets[2]],run))
      || (HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[1]],isJetWithL25JetBitPattern[leadingJets[1]],run) && HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[2]],isJetWithL25JetBitPattern[leadingJets[2]],run));
      }
    ...
    isJetWithBtagA[iJ]->fill(trgAccept,HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[iJ]],isJetWithL25JetBitPattern[leadingJets[iJ]],run),weight);
      } //if 
	} //for
     ...
     if (tripleOnlineBtagMatchOK2)  ///cut-based analysis
{
 ...
 // tpat counting
        ...
        int theTpat = 4 * int(HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[2]],isJetWithL25JetBitPattern[leadingJets[2]],run)) + 
        2 * int(HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[1]],isJetWithL25JetBitPattern[leadingJets[1]],run))
...
/*
              ...
              cout<<"mistagWeight="<<mistagWeight<<endl;
              cout<<"wbbpur="<<wbbpur<<endl;
              cout<<"wtpat="<<wtpat<<endl;
*/
...
/*
	...
	std::cout<<"44 After Fourth L25"<<std::endl;
		...
		hptdibbt[jetFlavorCodeNewCat(jetA)][0][ibtag][icateg]->Fill( ptJet[jetA], weight  * _SF_MC[ibtag]);
		hptdibbt[jetFlavorCodeNewCat(jetB)][1][ibtag][icateg]->Fill( ptJet[jetB], weight  *  _SF_MC[ibtag]);
	      }

	std::cout<<"55 After Fourth L25"<<std::endl;
	...
	std::cout<<"66 After Fourth L25"<<std::endl;
...
std::cout<<"Last line in the code"<<std::endl;
}
