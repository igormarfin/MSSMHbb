...
// 	templates.push_back(  templateRef("H",mjjEbtsignal[ibtag],0.001,2) );
    ...
    if (drawTemplateErrors) {
      theSummedTemplateHistProX->GetXaxis()->SetRange(3,25);
...
//       theSummedTemplateHistProX->Draw("HIST,SAME");
    ...
    //result->Draw("same");
    // draw marker
//     TMarker theDataMarker( xLegend+0.15, yLegend0-2*yIncrement, 20);
//     theDataMarker.SetNDC();
//     theDataMarker.Draw();
//     legend.SetTextColor(1);
//     legend.DrawLatex(xLegend+0.19,yLegend0-2*yIncrement,"Data");

    float yLegendMain = yLegend0 + 0.05;
    
    float xC = xLegend+0.1;
    float yC = yLegendMain-2*yIncrement;
    float xD = 0.09;
    float yD = 0.18;
//       TPave* thePave = new TPave( xC-xD, yC-yD,xC+xD,yC+yD,4);
//       thePave->SetNDC();
//       thePave->SetColor(templates[iTemplate].color);
    TLegend* leg = new TLegend(xC, yC-2*yD,xC+2*xD,yC);
    leg->SetFillColor(0);
    leg->SetBorderSize(1);
    leg->AddEntry( mjjEbtdataProX[ibtag], " Data" , "P" );
    for (int iTemplate=0; iTemplate<templates.size(); ++iTemplate) {
      templates[iTemplate].hist->SetFillColor( templates[iTemplate].hist->GetLineColor() );
      if ( templates[iTemplate].name != "H" ) {
	leg->AddEntry( templates[iTemplate].hist,(" "+templates[iTemplate].name).c_str(), "F"  );
      } else {
	leg->AddEntry( templates[iTemplate].hist,Form(" m_{#Phi}=%d GeV",signalMass[iSignal]), "F" );
      }
...
//       legend.DrawLatex(xLegend+0.15,yLegend0-0.15-iTemplate*yIncrement,sLegend.c_str());
    }        // print also the chi2
    leg->Draw();
    legend.SetTextColor(1);
    if (scenario == "LowMass2011") {
      legend.DrawLatex(xLegend+0.1,yLegendMain,"Low-Mass Scenario");
    } else {
      legend.DrawLatex(xLegend+0.1,yLegendMain,"Medium-Mass Scenario");
    }
    legend.DrawLatex(xLegend+0.1,yLegendMain-yIncrement,
		     Form("#chi^{2}/N_{DF} = %6.0f / %d",gTemplateFitter->GetChisquare(),
					   gTemplateFitter->GetNDF()));
    //legend.DrawLatex(xLegend,yLegend0-yIncrement,Form("Prob  = %6.4f ",gTemplateFitter->GetProb()));
    //legend.DrawLatex(xLegend,yLegend0-2*yIncrement,sbtag[ibtag].c_str());

    canvas->Print(Form("%s_templateFitStacked_%s_X.png",cMass.c_str(),sbtag[ibtag].c_str()));
    canvas->Print(Form("%s_templateFitStacked_%s_X.pdf",cMass.c_str(),sbtag[ibtag].c_str()));
...
//       legend.DrawLatex(xLegend+0.15,yLegend0-0.15-iTemplate*yIncrement,sLegend.c_str());
    ...
    canvas->Print(Form("%s_templateFitStacked_%s_Y.pdf",cMass.c_str(),sbtag[ibtag].c_str()));
