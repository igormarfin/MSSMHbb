...
void plotHiggsX() {
  ...
  bool useNP;
  getHbbCfg(scenario,useTemplateError,useNP);
...
#include "Analysis/Utilities/interface/HbbMass.h"
  ...
  for (int ibtag=0; ibtag<nbtag; ++ibtag) {
    ...
    Pos[jStati] = 1;
    yMax[jStati] = 0;
    if (scenario == "MediumMass2011") yMax[jStati] = double(400);
    ...
    Pos[jStati] = 1;
    yMax[jStati] = 0;
    if (scenario == "MediumMass2011") yMax[jStati] = double(55);
	...
	if (theError[iStati] != NULL) {
      ...
      if (yMax[iStati] > 0) {
	theYMax = yMax[iStati];
	std::cout << "Set yMax to " << yMax[iStati] << std::endl;
	if (Pos[iStati] == 0) {
	  theYMin = -yMax[iStati];
	}
      }
