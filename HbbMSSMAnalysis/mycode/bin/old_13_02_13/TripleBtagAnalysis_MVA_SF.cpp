...
if (_isOk) _afterMVA++;
    ...
    if (forceTripleOnlineBtagMatch) {
      ...
      if (nJet>=nSelJet ) {
    tripleOnlineBtagMatchOK2 = (HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[0]],run) && 
  HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[1]],run))
      || (HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[0]],run) && HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[2]],run))
      || (HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[1]],run) && HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[2]],run));
      }
    ...
    isJetWithBtagA[iJ]->fill(trgAccept,HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[iJ]],run),weight);
     ...
     if (tripleOnlineBtagMatchOK2)  ///cut-based analysis
{
 ...
 // tpat counting
        int theTpat = 4 * int(HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[2]],run)) + 
        2 * int(HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[1]],run))
