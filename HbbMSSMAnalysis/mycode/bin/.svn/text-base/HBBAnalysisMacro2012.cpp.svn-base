...
#include "Analysis/Utilities/interface/HBBTo4B_2012.h"
#include "Analysis/Utilities/interface/Trigger2012.h"
...
{
  // usage 
  // > HBBAnalysisMacro [filelist] [scenario]
  // example filelist is 
  ...
  // BTAG EFF CLASS
  const int nflav = 5; 
  std::string flavlabel[5] = {"udsg","c","b","cc","bb"};
  // define ovl labels
  const int nbtag = 4;
  ...
  float jetPtMin[3] = { 60,    53,   20.001};
  float jetPtMax[3] = {999,   999,   999};
  float jetEtaMax = 1.7;
  float dEta12Cut = 1.7;
  int minJet = 3;
  ...
  std::string btagOfflineEffFile("");
  std::string btagOnlineEffFile("");
  TString SVMassFileName("/data/user/rasp/Run/Hbb/svMass.root");
  TString SVMassOnlineFileName("");

  // TRIGGER LIST
  std::vector<TString> triggers;
  // HLT BTAG FILTER LIST
  std::vector<TString> hltBtagFilters;
  triggers.clear();
  hltBtagFilters.clear();
  if (Scenario == "MediumMass2012") {
    triggers.push_back("HLT_Jet60Eta1p7_Jet53Eta1p7_DiBTagIP3DFastPV_v");
    hltBtagFilters.push_back("hltBLifetimeL3FilterbbPhiL1FastJetFastPV");
    btagOfflineEffFile = std::string("/afs/naf.desy.de/user/j/jbehr/public/cms/btag/btagefficiencies_v104_53X_mediummass.root");
    btagOnlineEffFile =  std::string("/afs/naf.desy.de/user/j/jbehr/public/cms/btag/btagMatrixOnline-V57_53X_normal_mediummass.root");
    SVMassOnlineFileName = TString("/data/user/rasp/Run/Hbb/svMass1BL1FastJetFastPVHltMatch.root");
  }
  else if (Scenario == "HighMass2012") {
    triggers.push_back("HLT_Jet80Eta1p7_Jet70Eta1p7_DiBTagIP3DFastPV_v");
    hltBtagFilters.push_back("hltBLifetimeL3FilterbbPhiL1FastJetFastPV");
    btagOfflineEffFile = std::string("/afs/naf.desy.de/user/j/jbehr/public/cms/btag/btagefficiencies_v104_53X_highmass.root");
    btagOnlineEffFile =  std::string("/afs/naf.desy.de/user/j/jbehr/public/cms/btag/btagMatrixOnline-V57_53X_normal_highmass.root");
    SVMassOnlineFileName = TString("/data/user/rasp/Run/Hbb/svMass1BL1FastJetFastPVHltMatch.root");
    jetPtMin[0] = 80;
    jetPtMin[1] = 70;
  }
  else if (Scenario == "VeryHighMass2012") {
    triggers.push_back("HLT_Jet160Eta2p4_Jet120Eta2p4_DiBTagIP3DFastPVLoose_v");
    hltBtagFilters.push_back("hltBLifetimeL3FilterbbPhiLooseL1FastJetFastPV");
    btagOfflineEffFile = std::string("/afs/naf.desy.de/user/j/jbehr/public/cms/btag/btagefficiencies_v104_53X_veryhighmass.root");
    btagOnlineEffFile =  std::string("/afs/naf.desy.de/user/j/jbehr/public/cms/btag/btagMatrixOnline-V57_53X_loose_veryhighmass.root");
    SVMassOnlineFileName = TString("/data/user/rasp/Run/Hbb/svMass1BLooseL1FastJetFastPVHltMatch.root");
    jetPtMin[0] = 160;
    jetPtMin[1] = 120;
    jetEtaMax = 2.4;
    dEta12Cut = 999999;
  ...
  // btag eff
  bTagEff * bTagEffOffline = new bTagEff(btagOfflineEffFile,offlineBtag,flavlabel,sovl,nflav,nbtag);
  bTagEff * bTagRelEffOnline  = new bTagEff(btagOnlineEffFile,onlineBtag,flavlabel,sovl,nflav,nbtag);
  ...
  templateNames[0][3] = TString("C2bb");
  templateNames[0][4] = TString("B2bb");

  templateNames[1][0] = TString("bQb");
  templateNames[1][1] = TString("bCb");
  templateNames[1][2] = TString("bBb");
  templateNames[1][3] = TString("bC2b");
  templateNames[1][4] = TString("bB2b");

  templateNames[2][0] = TString("bbQ");
  templateNames[2][1] = TString("bbC");
  templateNames[2][2] = TString("bbB");
  templateNames[2][3] = TString("bbC2");
  templateNames[2][4] = TString("bbB2");
  ...
  TH1F * bTagTemplateH[4][3][5];
  TH1F * massTemplateH[4][3][5];
  TH2F * massBTagTemplateH[4][3][5];

  TH1F * bTagOnlineCorrectedTemplateH[4][3][5];
  TH1F * massOnlineCorrectedTemplateH[4][3][5];
  TH2F * massBTagOnlineCorrectedTemplateH[4][3][5];
  ...
  for (int isovl=0; isovl<4; ++isovl) {
	...
	massTemplateH[isovl][iCat][iFlav] = new TH1F("MassTemplate"+TString(sovl[isovl])+"_"+templateNames[iCat][iFlav]+"H","",30,0.,600.);
	...
	massOnlineCorrectedTemplateH[isovl][iCat][iFlav] = new TH1F("MassTemplate_OnlCorr_"+TString(sovl[isovl])+"_"+templateNames[iCat][iFlav]+"H","",30,0.,600.);
  ...
  for (int iFile=0; iFile<numberOfFiles; ++iFile) {
    ...
    HBBTo4B_2012 * hbbTo4b = new HBBTo4B_2012();
    hbbTo4b->Init(tree);

    Trigger2012 * trigger = new Trigger2012(file,triggers,hltBtagFilters);
    //    trigger->PrintOut();
	...
	std::cout << std::endl;
      ...
      numberOfJets = TMath::Min(numberOfJets,int(11));
	...
	bool puIDLoose = hbbTo4b->PUJetIDLoose[iJ];
	if (!puIDLoose) continue;
	...
	nJets++;
      ...
      // kinematic cuts
      float detaj = hbbTo4b->JetEta[j1] - hbbTo4b->JetEta[j2];
      float dphij = hbbTo4b->JetPhi[j1] - hbbTo4b->JetPhi[j2];
      if (dphij>TMath::Pi()) dphij -= 2*TMath::Pi();
      if (dphij<-TMath::Pi()) dphij += 2*TMath::Pi();      
      float deltaR12 = sqrt( dphij*dphij + detaj*detaj );
      float deltaEta = TMath::Abs(detaj);
      
      detaj = hbbTo4b->JetEta[j1] - hbbTo4b->JetEta[j3];
      dphij = hbbTo4b->JetPhi[j1] - hbbTo4b->JetPhi[j3];
      if (dphij>TMath::Pi()) dphij -= 2*TMath::Pi();
      if (dphij<-TMath::Pi()) dphij += 2*TMath::Pi();      
      float deltaR13 = sqrt( dphij*dphij + detaj*detaj );
      
      detaj = hbbTo4b->JetEta[j2] - hbbTo4b->JetEta[j3];
      dphij = hbbTo4b->JetPhi[j2] - hbbTo4b->JetPhi[j3];
      if (dphij>TMath::Pi()) dphij -= 2*TMath::Pi();
      if (dphij<-TMath::Pi()) dphij += 2*TMath::Pi();
      float deltaR23 = sqrt( dphij*dphij + detaj*detaj );
        

      if ( !(deltaR12 > 1.0 && deltaR23 > 1.0 && deltaR13 > 1.0 && deltaEta < dEta12Cut) ) continue;

      // ********
      // Triggers
      // ********
      bool bTagHlt1 = trigger->isHltBTagMatched(hbbTo4b->IsJetWithHltBtagBitPattern[j1]);
      bool bTagHlt2 = trigger->isHltBTagMatched(hbbTo4b->IsJetWithHltBtagBitPattern[j2]);
      bool bTagHlt3 = trigger->isHltBTagMatched(hbbTo4b->IsJetWithHltBtagBitPattern[j3]);
	...
	int massBin = sv->binNumber(diJetMass,massBins,binsMass);
        bTmassCSVTH[massBin]->Fill(float(nEvBTag),weight);
      }
	...
	for (int iCat=0; iCat<3; ++iCat) {
	  if (thisTpat==iCat || thisTpat == 3) { // online btag pattern must match offline
		...
		for (int iTag=0; iTag<3; ++iTag) {
		  ...
		  massTemplateH[iTagger][iCat][iFlv]->Fill(diJetMass,templateWeight);
		  float addWeight = 0;
		  for (int itpat=0; itpat<3; ++itpat) {
		    float wtpat = 1;
		  ...
		  massOnlineCorrectedTemplateH[iTagger][iCat][iFlv]->Fill(diJetMass,templateWeight * addWeight);
