...
_passKinematics++;
    ...
    for (int iJet=0; iJet<numberOfJets; ++iJet) {
      ...
      if (isJetWithHltBtagBitPattern[iJet] > 0) { //select any of the HLTBtag trigger objects
...
// 	  || (isJetWithHltBtag[leadingJets[1]] && isJetWithHltBtag[leadingJets[2]]);
        std::cout << "warning: FIXME. currently in this code a matching with any of the HLTBtag trigger objects is required! FIX THIS!" << std::endl;
        tripleOnlineBtagMatchOK2 = (isJetWithHltBtagBitPattern[leadingJets[0]] > 0 && isJetWithHltBtagBitPattern[leadingJets[1]] > 0)
