...
#include "Analysis/HbbMSSMAnalysis/interface/HbbPurityInit.cc"
  ...
  if ( doMVASel ) {
    ...
    if (seltreefile == NULL) {
      std::cout << "Opening of " << MVAselFilename.c_str() << " failed" << std::endl;
      return 0;
    }
...
#include "Analysis/Utilities/interface/HbbSelect.cc"
  ...
  if (firstFile.Contains("v8") || firstFile.Contains("v12") ||firstFile.Contains("RelOnlBtagCorr_v11") || firstFile.Contains("v15")) 
    {
...
//  const int nfc = 5;  
  ...
  // offline btag object
  bTagEff* bTagEffOffline = new bTagEff("/afs/naf.desy.de/user/j/jbehr/public/cms/btag/btagefficiencies_v16.root","offline",flavlabel,sbtag,nflav,nbtag);
  ...
  if(Scenario == "VeryHighMass2012") {
    ...
    bTagReleffOnline = new bTagEff("/afs/naf.desy.de/user/j/jbehr/public/cms/btag/btagMatrixOnline-V16_normal.root","online",flavlabel,sbtag,nflav,nbtag); 
  } else {
    bTagReleffOnline = new bTagEff("/afs/naf.desy.de/user/j/jbehr/public/cms/btag/btagMatrixOnline-V16_normal.root","online",flavlabel,sbtag,nflav,nbtag); 
  }
...
#include "Analysis/HbbMSSMAnalysis/interface/HbbPurityBook.cc"
  ...
  TrigHistArray* h_two_btags_on_leadingjets_pt_leading_jet = new TrigHistArray(&genericTriggerList,&triggerFilterList,
                                                                              "h_two_btags_on_leadingjets_pt_leading_jet",
                                                                              "h_two_btags_on_leadingjets_pt_leading_jet",
                                                                              50,0,500);
  TrigHistArray* h_twobtags_pt_leading_jet = new TrigHistArray(&genericTriggerList,&triggerFilterList,
                                                               "h_two_btags_pt_leading_jet",
                                                               "h_two_btags_pt_leading_jet",
                                                               50,0,500);
...
///Added,  to select using new steps deltaR13, deltaR12, deltaR23 and deltaEtaCut12
            ...
            //test impact of two btags on leading jet
            if(HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[0]],run) && HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[1]],run) ) {
              h_two_btags_on_leadingjets_pt_leading_jet->fill(trgSelect,ptJet[leadingJets[0]],weight);
            }
            const bool BtagMatch2outof3 = (HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[0]],run) && HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[1]],run))
              || (HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[0]],run) && HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[2]],run))
              || (HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[1]],run) && HLTBtagMatchObj.check(isJetWithHltBtagBitPattern[leadingJets[2]],run));
            if(BtagMatch2outof3) {
              h_twobtags_pt_leading_jet->fill(trgSelect,ptJet[leadingJets[0]],weight);
            }
