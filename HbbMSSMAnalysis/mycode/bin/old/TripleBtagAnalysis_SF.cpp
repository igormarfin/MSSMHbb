...
int main(int narg,char** varg) {
  ...
  std::string Scenario("LowMass2011");  // Higgs mass scenario
...
#include "Analysis/Utilities/interface/HbbSelect.cc"
  ...
  std::cout << "Jet pt cuts " << jetPtMin[0] << "  " 
...
//    }
    ...
    if ( (nJet>=nSelJet) && tripleOnlineBtagMatchOK ) {
      ...
      for (int iJ=0; iJ<nSelJet; ++iJ) {
	// jet kinematics
	ptJetA[iJ]->fill(trgSelect,ptJet[leadingJets[iJ]],weight);
	etaJetA[iJ]->fill(trgSelect,etaJet[leadingJets[iJ]],weight);
	phiJetA[iJ]->fill(trgSelect,phiJet[leadingJets[iJ]],weight);
	isJetWithBtagA[iJ]->fill(trgSelect,isJetWithHltBtag[leadingJets[iJ]],weight);
      }
