		...
		if (lcateg[icateg]) {
		  ...
		  for (int icorr=0; icorr<ncorr; ++icorr) {
			...
			for (int iSvM=0; iSvM<3; ++iSvM) {  // loop over possible svMassIndex values
			  ...
			  massBTagTemplateUncldA[ifc][ibtag][icateg][itpat]->fill(trgAccept,dijet_mass,float(evtBTag),weight * mistagWeight * wtpat * wtpatEvtBtag);
			  if (! (hasNegBTag[0] || hasNegBTag[1] )) {
			    massBTagTemplateCldA[ifc][ibtag][icateg][itpat]->fill(trgAccept,dijet_mass,float(evtBTag),weight * mistagWeight * wtpat * wtpatEvtBtag);
			  }
			  ...
			  if ( hasNegBTag[0] || hasNegBTag[1] ) {
			    massBTagTemplateNonbbRA[ifc][ibtag][icateg][itpat]->fill(trgAccept,dijet_mass,float(evtBTag),weight * mistagWeight * wtpat * wtpatEvtBtag * weightR);
			  }
			}
		      }
		    }
	...
	for (int ibtag=0; ibtag<nbtag; ++ibtag) {
	  for (int icateg=0; icateg<ncateg; ++icateg) {
	    ...
	    if ( (theBJetTag[ibtag][jetA]>btcut[ibtag]) &&  (theBJetTag[ibtag][jetB]>btcut[ibtag]) ) {
	      mDibBtcutA[ibtag][icateg]->fill(trgAccept,dijet_mass, weight );
	      for (int iJ=0; iJ<nSelJet; ++iJ) {
		ptDibBtcutA[ibtag][icateg][iJ]->fill(trgAccept,ptJet[leadingJets[iJ]],weight);
	      }
	    }
		...
		* bTagEffOffline->eff(theFlav[shift[icateg+2]],sbtag[ibtag].c_str(),ptJet[jetB],etaJet[jetB]);
	      mDibBtweightA[ibtag][icateg]->fill(trgAccept,dijet_mass, weight * wDoubleBtag);
	      for (int iJ=0; iJ<nSelJet; ++iJ) {
		ptDibBtweightA[ibtag][icateg][iJ]->fill(trgAccept,ptJet[leadingJets[iJ]],weight * wDoubleBtag );
	      }
